...
## Collect most probable DAGs

```{r, error = TRUE}
flat_eq_chains <- FlattenChains(eq_chains)
dag_collection <- CollectDags(flat_eq_chains)
```

Calculate pairwise edge probabilities using the DAG collection. 

```{r, error = TRUE}
dag_arr <- simplify2array(dag_collection$dag)
p_vec <- exp(dag_collection$log_norm_score)
for (i in 1:length(p_vec))
  dag_arr[, , i] <- dag_arr[, , i]*p_vec[i]
p_edge <- apply(dag_arr, c(1, 2), sum)
knitr::kable(p_edge)
```
