...
Acceptance per proposal. This is often incredibly slow, so I do wonder if we've
...
## Collect most probable DAGs and perform inference for features of interest

```{r, error = TRUE}
flat_eq_chains <- FlattenChains(eq_chains)
chain_collection <- CollectUniqueObjects(flat_eq_chains)
```

Get most probable DAGs. I'm just plotting the CPDAG of the first map DAG found here.

```{r, error = TRUE}
map_dags <- GetMAPDAG(chain_collection)

bn_map_dag <- bnlearn::empty.graph(colnames(data))
bnlearn::amat(map_dag) <- map_dags$dag[[1]]

plot(bnlearn::cpdag(map_dag))
```

Calculate marginalised pairwise edge probabilities using the DAG collection. 

```{r, error = TRUE}
p_edge <- CalculateEdgeProbabilities(chain_collection)
knitr::kable(p_edge)
```
