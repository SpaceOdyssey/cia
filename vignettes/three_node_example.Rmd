...
For the split join proposal...

```{r, error = TRUE}
dags_col <- simplify2array(dags[[1]])
dags_diff <- dags_col[, , 2:(n_results - n_burnin)] - dags_col[, , 1:(n_results - n_burnin - 1)]
dags_chg <- apply(abs(dags_diff), 3, sum) == 0

accepted_sj <- ((chains[[1]]$proposal_used[n_burnin:n_results] == 'split_join')
                & (chains[[1]]$accept[n_burnin:n_results] == 1)
                & (dags_chg))
```

For the node move proposal...
