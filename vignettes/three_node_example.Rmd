...
```{r, error = TRUE}
scorer <- list(scorer = memo::memo(BNLearnScorer, memo::lru_cache(10000)), parameters = list(data = data, type = 'bde'))
...
```{r, error = TRUE}
n_burnin <- 500
eq_chains <- PostProcessChains(chains, n_burnin = n_burnin)
...
```{r, error = TRUE}
prop_accept <- CalculateAcceptanceRates(eq_chains, group_by = c('proposal_used'))
