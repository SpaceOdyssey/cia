...
#     chain <- SampleChain(n_results, partitioned_nodes, 
#                          transition = PartitionMCMC(), scorer = scorer)
#     chains[[i]] <- chain
#   }
# )
# ```

This get n_results samples for each core. 
...
chain_info <- dplyr::bind_cols(
  proposal_used = unlist(chains[[1]]$proposal_info, use.names = FALSE), 
  accept = unlist(chains[[1]]$mcmc_info, use.names = FALSE)
  )
...
for (i in 1:min(nrow(bw1), 8)) {
  hash_focus_dag <- bw1$hash_dag[i]
  ...
  t <- paste('p_est: ', round(bw1$p[i], 3), ', p_true: ', round(bw1$p_true[i], 3), sep = '')
